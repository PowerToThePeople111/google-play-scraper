...
function createConnectionPool(){
  ...
  for(let confIdx in connection_lists) {
    ...
    var con = {
      ...
      controlPort: configElements[0].split(" ")[1],
    }
    pool.push(con);
    console.log(con);
...
function applyCon(pool, conIdx, tor){
  ...
  var con = pool[conIdx];
  tor.setTorAddress("localhost", con.socksPort);
  tor.TorControlPort.port("localhost", con.controlPort);
  tor.TorControlPort.password("localhost", "dadascience");
  console.log(con.socksPort + " - " + con.controlPort)
  printTOR_IP();
