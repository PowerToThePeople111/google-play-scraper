...
function applyCon(pool, conIdx, tr){
  //tr.TorControlPort.password = 'dadascience';
  var con = pool[conIdx];
  tr.setTorAddress("localhost", con.socksPort);
  tr.TorControlPort.port("localhost", con.controlPort);
...
function doRequest (opts, limit) {
  ...
  requests = requests - 1;
  tr = applyCon(conPool, requests % conPool.length, tr)
